...
def exact_sum_2(tuple_list, tex = True):
    # Computes and builds the exact value of the series; input a second parameter "True" for LaTeX, "False" for plain text.
    S = 0
    for i in tuple_list:
        for j in tuple_list:
            N = i[2]+conjugate(j[2])
...
def lin_comb(rational_expression):
    # Expresses the summand as a linear combination of non-negative rational functions.
...
def rational_func(rational_expression):
    # Outputs the non-negative functions in the linear combination.
...
def rational_SUM_1(rational_expression, sub_approx = False, tex = False):
    # Outputs the exact value of the series; input a second parameter "True" to display approximate values each time a non-negative rational function is fed into the SUM function, "False" to hide the approximations; input a third parameter "True" for LaTeX, "False" for plain text.
    list_of_nonneg = lin_comb(rational_expression)
    answer = 0
    for i in list_of_nonneg:
...
def rational_SUM_2(rational_expression, sub_approx = False, tex = False):
